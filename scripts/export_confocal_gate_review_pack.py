...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export a confocal moderate-vs-relaxed gate review pack.")
    parser.add_argument("--config", default="configs/default.yaml")
    parser.add_argument("--baseline-variant", default=DEFAULT_BASELINE_VARIANT)
    parser.add_argument("--relaxed-variant", default=DEFAULT_RELAXED_VARIANT)
    parser.add_argument("--images", nargs="+", default=DEFAULT_FOCUS_IMAGES)
    parser.add_argument("--output-dir")
    parser.add_argument("--write-zip", action="store_true")
    args = parser.parse_args()

    review_summary, summary, paths = export_confocal_gate_review_pack(
        load_config(args.config),
        baseline_variant=args.baseline_variant,
        relaxed_variant=args.relaxed_variant,
        images=args.images,
        output_directory=args.output_dir,
        write_zip=args.write_zip,
    )
    print(f"baseline_variant: {summary['baseline_variant']}")
    print(f"relaxed_variant: {summary['relaxed_variant']}")
