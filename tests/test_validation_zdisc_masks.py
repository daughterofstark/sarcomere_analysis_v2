...
from sarcomere_analysis.validation_zdisc_masks import (
    axial_angular_error_deg,
    build_zdisc_mask_validation,
    validate_zdisc_masks,
)
    ...
    return pd.DataFrame(
        [
            ...
            },
            {
                ...
                "image_id": "3.032-1",
                "donor_id": "3.032",
                "patch_id": "3.032-1_p00001",
                "annotation_status": "ignore_only",
                "zdisc_pixel_fraction": 0.0,
                "has_zdisc_labels": False,
...
def synthetic_patches(include_missing: bool = False, donor_mismatch: bool = False) -> pd.DataFrame:
    rows = [
        {
            "image_id": "2.007-1",
            "donor_id": "2.007",
            "patch_id": "2.007-1_p00001",
            "patch_oop": 0.8,
            "patch_mean_orientation_deg": 5.0,
        ...
        rows.append(
            {
                ...
                "patch_id": "3.032-1_p00001",
                "patch_oop": 0.4,
                "patch_mean_orientation_deg": 60.0,
                "valid_for_orientation": True,
                "invalid_reason": "ok",
            }
        )
...
def test_join_preserves_annotation_rows() -> None:
    matched, summary = build_zdisc_mask_validation(synthetic_annotations(), synthetic_patches())

    assert len(matched) == 3
    assert summary["total_annotation_masks"] == 3


def test_unmatched_patches_are_reported() -> None:
    matched, summary = build_zdisc_mask_validation(synthetic_annotations(), synthetic_patches(include_missing=True))

    assert summary["unmatched_rows"] == 1
    assert "unmatched_patch" in set(matched["validation_match_status"])


def test_donor_id_mismatch_is_detected() -> None:
    matched, summary = build_zdisc_mask_validation(synthetic_annotations(), synthetic_patches(donor_mismatch=True))
...
def test_orientation_metrics_skip_nan_manual_orientations() -> None:
    _, summary = build_zdisc_mask_validation(synthetic_annotations(), synthetic_patches())

    assert summary["n_orientation_pairs"] == 2
    assert summary["median_axial_error_deg"] == 12.5


def test_group_medians_compute_for_zdisc_empty_ignore_groups() -> None:
    _, summary = build_zdisc_mask_validation(synthetic_annotations(), synthetic_patches())
    medians = summary["oop_medians_by_annotation_status"]

    assert medians["zdisc_labeled"] == 0.8
    assert medians["empty"] == 0.2
    assert medians["ignore_only"] == 0.4


def test_spearman_is_skipped_safely_when_n_too_small() -> None:
    _, summary = build_zdisc_mask_validation(synthetic_annotations(), synthetic_patches(), min_n_for_correlation=10)
...
def test_summary_json_is_serializable(tmp_path: Path) -> None:
    ...
    ann_dir = tmp_path / "results" / "zdisc_annotation"
    tables.mkdir(parents=True)
    ann_dir.mkdir(parents=True)
    annotation_path = ann_dir / "zdisc_annotation_features.csv"
    patch_path = tables / "features_per_patch.csv"
    synthetic_annotations().to_csv(annotation_path, index=False)
    synthetic_patches().to_csv(patch_path, index=False)

    _, _, paths = validate_zdisc_masks(cfg, annotation_features=annotation_path, patch_features=patch_path)
    loaded = json.loads(paths["summary_json"].read_text(encoding="utf-8"))

    assert loaded["total_annotation_masks"] == 3
...
def test_input_tables_are_not_modified() -> None:
    ...
    build_zdisc_mask_validation(annotations, patches)
...
def test_donor_id_preserved_as_string() -> None:
    matched, _ = build_zdisc_mask_validation(synthetic_annotations(), synthetic_patches())
