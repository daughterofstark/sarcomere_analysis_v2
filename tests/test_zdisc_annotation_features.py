...
from sarcomere_analysis.zdisc_annotation_features import (
    annotation_status,
    estimate_mask_orientation,
    extract_one_mask_features,
    extract_zdisc_annotation_features,
)
...
def make_feature_set(tmp_path: Path, masks: list[np.ndarray]) -> tuple[dict, Path]:
    ...
    for idx, mask in enumerate(masks, start=1):
        ...
        mask_path = mask_dir / f"{annotation_id}_mask.png"
        Image.fromarray(np.full(mask.shape, 80, dtype=np.uint8)).save(image_path)
...
def test_empty_mask_returns_annotation_status_empty(tmp_path: Path) -> None:
    cfg, index_path = make_feature_set(tmp_path, [np.zeros((10, 10), dtype=np.uint8)])

    features, summary, _ = extract_zdisc_annotation_features(cfg, index_path=index_path)
...
def test_label_1_mask_returns_has_zdisc_labels_true(tmp_path: Path) -> None:
    ...
    mask[2:5, 2:5] = 1
    cfg, index_path = make_feature_set(tmp_path, [mask])

    features, _, _ = extract_zdisc_annotation_features(cfg, index_path=index_path)
...
def test_connected_components_counted_correctly(tmp_path: Path) -> None:
    ...
    features, _, _ = extract_zdisc_annotation_features(cfg, index_path=index_path, min_zdisc_pixels=1)
...
def test_orientation_for_synthetic_diagonal_line_is_finite_and_plausible() -> None:
    ...
    assert result["manual_mask_orientation_confidence"] > 0.9
...
def test_orientation_returns_nan_for_too_few_pixels() -> None:
    ...
    assert result["reason_not_estimable"] == "too_few_zdisc_pixels"
...
def test_summary_json_is_serializable(tmp_path: Path) -> None:
    ...
    _, _, paths = extract_zdisc_annotation_features(cfg, index_path=index_path)
...
def test_input_masks_are_not_modified(tmp_path: Path) -> None:
    ...
    extract_zdisc_annotation_features(cfg, index_path=index_path)

    assert mask_path.read_bytes() == before


def test_annotation_status_helper() -> None:
    assert annotation_status(0, 0) == "empty"
    assert annotation_status(3, 0) == "zdisc_labeled"
    assert annotation_status(0, 3) == "ignore_only"
    assert annotation_status(3, 3) == "mixed"


def test_extract_one_mask_preserves_identifiers_as_strings(tmp_path: Path) -> None:
    mask = np.zeros((10, 10), dtype=np.uint8)
    cfg, index_path = make_feature_set(tmp_path, [mask])
    _ = cfg
    row = pd.read_csv(index_path, dtype=str).iloc[0]

    features = extract_one_mask_features(row)
